...
impl Camera {
    pub fn new(position: Point<f64, 3>) -> Self {
        Camera {
            position,
    ...
    pub fn view_transform(&self) -> Transform {
        Transform::builder()
            .translate(self.position.vector_to(&Point::new([0.0, 0.0, 0.0])))
    ...
    pub fn move_relative(&mut self, movement: Vector<f64, 3>) {
        let movement: Vector<f64, 3> = {
            let transform = Transform::builder()
                .translate(self.position.vector_from(&Point::new([0.0, 0.0, 0.0])))
                .rotate_about_x(self.orientation[RotationAxis::Roll])
                .rotate_about_y(self.orientation[RotationAxis::Pitch])
                .rotate_about_z(self.orientation[RotationAxis::Yaw])
                .build_affine();

            let mut homogenous: Vector<f64, 4> = movement.promote();
